...
%% Calculate Ratio of Blank and Transmission Scan
sinoTotalR = zeros(Nparts,Nbins,Nproj,Nslices);
for j=1:Nparts
  SinoTrans(:,:,:) = sinoTotalT(j,:,:,:);
  SinoBlank(:,:,:) = sinoTotalB(j,:,:,:);
  ...
  sinoTotalR(j,:,:,:) = SinoRatio(:,:,:);
  name = strcat(filenameT,'_',num2str(j));
  newName = strcat('sino_SSRB_ratio_', name, '.raw');
  fid = fopen(newName,'w');
...
name = strcat('SSRB_trans_nans_',filenameT,'.raw');
fid = fopen(name,'w');
...
TransComplete = TransComplete - 13*EmissComplete;
