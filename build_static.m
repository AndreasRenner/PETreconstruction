function build_sino_static(filename,Ncs,Nr,Na,Ns)

NRAD=str2num(Nr);           % Number of radial bins
NANG=str2num(Na);           % Number of projections
Nsinos=str2num(Ns);         % Number of 3D sinogram planes
Ncounts=str2num(Ncs);       % Number of counts in the LIST file

paso_t      =   0.001;
sino_dims   =   [NRAD,NANG,Nsinos];

    fid=fopen(filename,'r');
    dlist=fread(fid,[Ncounts],'uint32');    
    fclose(fid);
    ...
    for i=1:length(dlist)
        ...
        elseif (dlist(i)>=(2^31))&&(dlist(i)<2684354560)
            ...
            if mod(Ttag(T)-(2^31),1000)==0
                fprintf('Prompts in second %s: %u\r', num2str((T-1)/1000), (length(ptag)-templength));
                templength=length(ptag);
            end
            T=T+1;
        else
            Ftag(F)=dlist(i);
    ...
    fprintf('Randoms\t:\t%s\r\n',num2str(length(rtag)));   
    fprintf('\r\n');
    fprintf('Time marks\t:\t%s\r\n',num2str(length(Ttag)));
    fprintf('Dead-time marks\t:\t%s\r\n',num2str(length(Dtag)));    
    ...
    fprintf('Total events analyzed\t:\t%s\r\n',num2str(Nevnts));
    
    if(Nevnts~=Ncounts);
        fprintf('CAUTION!! Events analyzed is not = to initial number of counts!!');
    end
