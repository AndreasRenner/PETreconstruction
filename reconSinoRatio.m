function reconSinoRatio(nameBlank,nameTrans,nameMask,reconMethod)
...
filenameMask  = strcat('SSRB_',nameMask,nameTrans,'.raw');
...
name = strcat('SSRB_333ratio_',nameTrans,'.raw');
fid = fopen(name,'w');
...
name = strcat('SSRB_333ratio333_SEG_',nameTrans,'.raw');
fid = fopen(name,'w');
fwrite(fid,SinoRatio,'float32');
fclose(fid);
