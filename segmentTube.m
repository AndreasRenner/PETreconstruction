...
%  [~,xp2] = radon(mask(:,:,1),theta);
%  m   = size(xp2,1);
%  mmm = (m-Nbins)/2-0.5;
%  clear R2;
%  clear xp2;
%  parfor k=1:Nslices
%    [R2,~] = radon(mask(:,:,k),theta);
%    for l=1:Nbins 
...
maskname   = '997ratio_mask_';
reconSinoRatio(reconblank,recontrans,maskname,'FBP');
reconSinoRatio(reconblank,recontrans,maskname,'OSEM');
